...
void CFire::attackMonster(CAttack* attack, CMonster* target)
{
	...
	else
	{
		...
		//If burned() returns true AND the target has no negative state due to a specialty AND the target isn't Fire
		if (burned() && target->getterStateMonster() == state::Healthy && target->getterTypeMonster() != type::Fire)
		{
			target->setterStateMonster(state::Burned);
			target->setternbTurnUnderState(0);
			std::cout << target->getterNameMonster() << " is burned" << std::endl;
...
bool CFire::burned()
{
	return randomLowerThan(0, 1, m_burn);
}

void CFire::setterBurn(float burn)
{
	m_burn = burn;
}
